...
flowchart LR
    ...
    subgraph W4["Week 4 · M5 — Monitoring"]
        ...
        PLOG --> DRIFT[Drift simulation<br/>report]
...
## Week 4 (M5) — monitoring & drift

```powershell
podman-compose up --build                      # api + Prometheus (:9090) + Grafana (:3000)
python -m dvc repro drift_simulation           # simulate drift, write the drift report
python -m dvc repro retraining_decision        # apply the trigger rule, write the verdict
```

Grafana auto-loads the **ETA API Overview** dashboard from the Prometheus datasource;
both are provisioned under [monitoring/grafana](monitoring/grafana). The API's `/metrics`
endpoint exposes request rate, latency histograms, prediction counts and the absolute
error histogram fed by `/feedback`.

**Drift is a controlled intervention, not a guess.** Because the data-generating process
is ours, `monitoring.drift.scenario: festival_surge` shifts congestion and trip duration
by a known amount, so a detected drift is ground truth the detector is checked against.
Swap in `monsoon`, `new_zone` or `concept_drift` to rehearse a different failure mode.

Two signals are simulated because the two ways a deployed model fails are different, and
a monitor watching only one is blind to the other:

| Signal | Watches | Method |
| --- | --- | --- |
| **Performance drift** | input→output relationship (concept) | model MAE on the drifted window vs a baseline window |
| **Feature drift** | input distribution (covariate) | Week 1 Level 3 checks (KS / chi² / mean-shift) vs the training baseline |

A third **live_error** signal reads the mean absolute error of recently *labelled*
predictions from the serving log; below `min_labelled` it abstains rather than voting on
thin data. On the committed run, `festival_surge` drove MAE from **3.51 → 10.88 min
(×3.10)** and flagged **6** feature columns, and 60 fed-back trips gave a live MAE of
**112.8 min** — all three signals failed
([drift report](reports/monitoring/drift_report.md) ·
[decision](reports/monitoring/retraining_decision.md)).

**The retraining trigger is one auditable rule**, tested branch by branch in
[tests/test_monitoring.py](tests/test_monitoring.py): retrain when at least
`monitoring.retrain.fail_signals_to_trigger` signals reach *fail*, otherwise observe.
A written-down rule is auditable; a clever one living in someone's head is not. Every
threshold sits in [params.yaml](params.yaml) under `monitoring`, so tightening the trigger
invalidates exactly the two Week 4 stages and nothing upstream.

## Continuous integration

CI runs via [.github/workflows/ci.yml](.github/workflows/ci.yml)
(lint → regenerate data → tests) on a **self-hosted** runner hosted locally as a Podman
container from [runner/](runner/):

```powershell
# 1. Build the image
podman build -t eta-gh-runner -f runner/Containerfile runner

# 2. Get a fresh registration token from GitHub:
#    Repo → Settings → Actions → Runners → New self-hosted runner

# 3. Start the runner (pass the token at runtime — never commit it)
podman run -d --name eta-gh-runner --restart=on-failure `
  -e REPO_URL="https://github.com/2025paml518-coder/ETA-Prediction-ML-Pipeline" `
  -e RUNNER_TOKEN="<REGISTRATION_TOKEN>" `
  eta-gh-runner

podman logs -f eta-gh-runner    # should end with "Listening for Jobs"
```

Manage it with `podman stop` / `start` / `rm -f eta-gh-runner`. Stopping de-registers the
runner automatically; the registration token is short-lived, so re-registering needs a new
one. The workflow targets this runner via `runs-on: [self-hosted, linux, x64, podman]`, so
it must be online for CI to run.
...
- [x] **Week 4 · M5** — drift simulation, Prometheus/Grafana monitoring, retraining trigger
